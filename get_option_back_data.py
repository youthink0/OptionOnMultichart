...
def group_by_strike_price(txo_df, date_tmp, transaction_second_index):
    ...
    for i in range(18000) :
        ...
        for strike_price_start_value in np.unique(txo_df['履約價格'].astype(int)):
            ...
            #####對每個履約價處理每日日盤逐筆交易#####
            #function
            price_name = process_to_import_form(txo_df, strike_price_start_value, transaction_second_index, date_tmp)
...
def find_price_flat_sum(txo_df, min_strike_price, transaction_second_index, date_tmp):
    ...
    #####把價平履約價的格式準備好#####
    #function
    price_flat_sum = process_to_import_form(txo_df, min_strike_price, transaction_second_index, date_tmp)
...
def process_by_time_gap(origin_df, transaction_second_index, day_flag):
    ...
    if day_flag == 1 or day_flag == 0:
        ...
        min_strike_price = group_by_strike_price(txo_df, date_tmp, transaction_second_index)
    
    #####找完價平履約價後,接著就生成該履約價的df#####
    #function
    price_name = find_price_flat_sum(txo_df, min_strike_price, transaction_second_index, date_tmp)
