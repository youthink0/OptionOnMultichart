...
def group(store):
    ...
    #價平差方面是put - call #這邊操作非常難
    ################################
    store['成交價格'] = store['成交價格'].astype(int)
    store.to_csv("s.csv")
    a = store[ store['買賣權別'] == '    C     ' ].成交價格 * 2
    new_index = store.index
    a = a.reindex(new_index, fill_value = 0) #把a缺失的index補足，長度跟store要相同
    #print(a.head)
    store['成交價格'] = store['成交價格'].subtract(a) #把call中成交價格全部取負號   
    #print( store )
    ###############################
    
    store = store.groupby(['排序依據']).成交價格.sum() #價平差依據上述操作後，這邊其實是會相減
...
def op_out(pri, Filename): #輸出成csv
    my_folder_path = "C:/Users/a0985/OneDrive/Desktop/k" #到價平差資料夾
    file_address = my_folder_path + "/" + "_op_price_" + Filename + "_sub.csv"
    ...
    if os.path.isfile(file_address) : 
        ...
        pri.to_csv(file_address, index = False)
    #print(Filename + 'ok')
    return; #end
