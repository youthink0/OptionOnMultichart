...
def erase_redundant_space_and_value(origin_df):
    origin_df.rename(columns={
                     ' 成交日期': '成交日期', '          商品代號': '商品代號', '        履約價格': '履約價格'}, inplace=True)
    origin_df.rename(columns={
                     '                                                      到期月份(週別)': '到期月份(週別)'}, inplace=True)
    origin_df.rename(columns={'        買賣權別': '買賣權別',
                              '      成交時間': '成交時間', '          成交價格': '成交價格'}, inplace=True)
    origin_df.rename(columns={
                     '         成交數量(B or S)': '成交數量(B or S)', '     開盤集合競價 ': '開盤集合競價'}, inplace=True)
    ...
    origin_df['到期月份(週別)'] = origin_df['到期月份(週別)'].astype(
...
def get_transaction_second_df_to_price_name(price_name, transaction_second_index):
    ...
    print(price_name)
    aggregation_functions = {'call': 'sum', 'put': 'sum',
                             'call_Volume': 'sum', 'put_Volume': 'sum'}  # 依照相同時間段合併row

    #如此每一時間皆有put&call val
    price_name = price_name.groupby(
        '成交時間', as_index=True).aggregate(aggregation_functions)
...
def get_today_rpt(rpt_name):
    ...
    if re != 200:
        ...
        file_dir = '/Users/sonicwang/fin/OptionOnMultichart'  # 解壓後的文件放在該目錄下
...
def read_origin_data(time_format_in_origin_file_name):
    ...
    fname = '/Users/sonicwang/fin/OptionOnMultichart/OptionsDaily_' + \
...
def preprocess(origin_df, day_flag):
    if ' 交易日期' in origin_df.columns:  # 把原始檔中不同column name統一成成交日期
        ...
        origin_df.drop(labels=[' 交易日期'], axis='columns',
                       inplace=True)  # delete多餘欄位
    ...
    txo_df.drop_duplicates(['成交日期', '履約價格', '買賣權別', '成交時間'],
                           'first', inplace=True)  # 刪掉指定條件重複列
...
def process_to_import_form(txo_df, strike_price_value, transaction_second_index, date_tmp):
    price_name = txo_df[txo_df['履約價格'] == str(
        strike_price_value)]  # 依照不同履約價分類df
...
def tickToMin(complete_strike_price_data):

    complete_strike_price_data['Time'] = complete_strike_price_data['Time'].apply(
        lambda x: x // 100)
    print(complete_strike_price_data.tail())
    row_iter = complete_strike_price_data.iterrows()
    ...
    for idx, row in row_iter:
        if row['Time'] == first['Time']:
            ...
            complete_strike_price_data.loc[first_idx, 'O'] = O
            complete_strike_price_data.loc[first_idx, 'H'] = H
            complete_strike_price_data.loc[first_idx, 'L'] = L
            complete_strike_price_data.loc[first_idx, 'C'] = C
            complete_strike_price_data.loc[first_idx, 'Volume'] = V
            first = row
            first_idx = idx
            O = H = L = C = first['Price']
            V = first['Volume']
    ...
    complete_strike_price_data.drop_duplicates(subset=['Time'], inplace=True)
    complete_strike_price_data['Time'] = complete_strike_price_data['Time'].apply(
        lambda x: x * 100)
    complete_strike_price_data = complete_strike_price_data.drop(['Price'], axis=1)
    complete_strike_price_data = complete_strike_price_data[[
        'Date', 'Time', 'O', 'H', 'L', 'C', 'Volume']]
    return complete_strike_price_data
...
def get_import_form(price_name, strike_price_start_value):  # 整理成QM讀取CSV的格式
    ...
    # complete_strike_price_call_data = tickToMin(complete_strike_price_call_data)
    #####依照履約價個別輸出成csv#####
    #function
    ...
    complete_strike_price_put_data = tickToMin(complete_strike_price_put_data)
    #####依照履約價個別輸出成csv#####
    #function
    output_to_csv_by_strike_price(
    ...
    ###
    complete_strike_price_data = price_name[[
        'Date', 'Time', 'Price', 'Volume']]
...
def output_to_csv_by_strike_price(complete_strike_price_data, Filename):
    print(Filename)
    print(complete_strike_price_data.head(10))
    #####設定輸出位置#####
    my_folder_path = "/Users/sonicwang/fin/OptionOnMultichart/op_data"  # 到價平和資料夾
...
def process_by_time_gap(origin_df, transaction_second_index, day_flag):
    ...
    txo_df, date_tmp = preprocess(origin_df, day_flag)
